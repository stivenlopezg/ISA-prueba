...
def export_data(df: pd.DataFrame, file_path: str, with_header: bool = False):
    ...
    if with_header:
        return df.to_csv(file_path, sep=';', index=False, header=True, decimal='.')
    else:
        return df.to_csv(file_path, sep=';', index=False, header=False, decimal='.')
